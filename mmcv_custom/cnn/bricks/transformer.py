...
class MultiScaleDeformableAttentionCustom(BaseModule):
    ...
    def __init__(self,
        ...
        self.mask_object_query = mask_object_query
    ...
    def forward(self,
        ...
        if self.mask_object_query:
            attn_mask = torch.zeros((num_query, self.num_heads, self.num_levels, self.num_points),
                                    dtype=torch.bool, device=attention_weights.device)  # broadcasted over bs
            attn_mask[:, :, -1, :] = True  # the last feature level is object queries
            assert attn_mask.dtype == torch.bool, \
                'only support torch.bool as attn_mask for now, got {}'.format(attn_mask.dtype)
...
class BaseTransformerLayerCustom(BaseModule):
    ...
    def __init__(self,
        ...
        super(BaseTransformerLayerCustom, self).__init__(init_cfg)
        ...
        for operation in operation_order:
            if operation in ['prompt_self_attn', 'self_attn', 'cross_attn']:
                if operation in ['prompt_self_attn'] and 'shared' in attn_cfgs[index]:
                    index += 1
                    continue
    ...
    def forward(self,
                ...
                key_padding_mask=None,
                **kwargs):
        """Forward function for `DeformableDetrTransformerEncOnlyEncoder`.
        ...
        **kwargs contains some specific arguments of attentions.
        ...
        for layer in self.operation_order:
            if layer == 'self_attn':
                ...
                object_queies_start_idx = kwargs['level_start_index'][-1]
                ...
                if 'prompt_self_attn_layer' not in kwargs:
                    object_queies = self.attentions[attn_index](
                        object_queies,
                        temp_key,
                        temp_value,
                        inp_residual_propmt if self.pre_norm else None,
                        query_pos=query_pos_propmt,
                        key_pos=query_pos_propmt,
                        attn_mask=attn_masks[attn_index],
                        key_padding_mask=query_key_padding_mask_propmt,
                        **kwargs)
                    attn_index += 1
                else:
                    # the shared prompt encoder is passed as kwargs
                    # see 'DeformableDetrTransformerEncOnlyEncoder'
                    object_queies = kwargs['prompt_self_attn_layer'](
                        object_queies,
                        temp_key,
                        temp_value,
                        inp_residual_propmt if self.pre_norm else None,
